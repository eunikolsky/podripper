...
DONE_RIP_OUTPUT_DIR="$BASE_OUTPUT_DIR/$RIP_DIR_NAME"
# The output directory for the current rip.
RAW_RIP_OUTPUT_DIR="$DONE_RIP_OUTPUT_DIR/incomplete/"

[[ -d "$RAW_RIP_OUTPUT_DIR" ]] || mkdir -p "$RAW_RIP_OUTPUT_DIR"

cd "$BASE_OUTPUT_DIR"

# remove `mp3`s older than 10 days to clean up space
find "$DONE_RIP_OUTPUT_DIR" -depth -maxdepth 1 -type f -name '*.mp3' -mtime +10 -delete || true
...
while (( $( date '+%s' ) < "$END_TIMESTAMP" )); do
  streamripper "$STREAM_URL" --quiet -d "$RAW_RIP_OUTPUT_DIR" -s -r -R 3 -a -A -o version -t -m 5 -M 1000 -l "$DURATION_SEC"
...
if [[ -n "$( ls -A "$RAW_RIP_OUTPUT_DIR" )" ]]; then
  year="$( date '+%Y' )"
  for rip in "$RAW_RIP_OUTPUT_DIR"/*.mp3; do
    pod_title="$( echo "$rip" | sed -nE 's/.*([0-9]{4})_([0-9]{2})_([0-9]{2})_([0-9]{2})_([0-9]{2})_([0-9]{2}).*/\1-\2-\3 \4:\5:\6/p' )"
    if ! ffmpeg -hide_banner -i "$rip" -vn -v warning -codec:a libmp3lame -b:a 96k -metadata title="$pod_title" -metadata artist="$POD_ARTIST" -metadata album="$POD_ALBUM" -metadata date="$year" -metadata genre=Podcast "$DONE_RIP_OUTPUT_DIR/$( basename -s .mp3 "$rip" )_enc.mp3"; then
      echo "reencoding $rip failed; moving the source"
      mv "$rip" "$DONE_RIP_OUTPUT_DIR/$( basename -s .mp3 "$rip" )_src.mp3"
    else
      rm -f "$rip"
    fi
  done
  ...
  rm -f "$RAW_RIP_OUTPUT_DIR"/*.cue
else
  echo "no files in $RAW_RIP_OUTPUT_DIR"
fi
